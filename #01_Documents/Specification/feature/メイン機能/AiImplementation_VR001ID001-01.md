...
- [x] **検索・一覧表示機能**: DataGridを部分一致フィルタリング。一覧は「最終作成日（LastCreatedDate）」の降順でソートし、一番上の項目を自動選択する。絞り込み・追加・削除時にNo列を1から再採番するロジック。
...
### STEP 5: メッセージ文字列の resx 一元管理化
- [ ] `Properties` フォルダを作成し、その中に `Resources.resx` および `Resources.Designer.cs` を新規作成する。
- [ ] 以下の「4. エラーメッセージ定義要件」に記載されている全てのメッセージ文字列、および各種ダイアログのタイトル（「エラー」「確認」「完了」など）、確認メッセージ（「登録を解除しますか？」「ワークスペースの作成が完了しました。」）を `Resources.resx` に定義する。
- [ ] `MainViewModel.cs` および `WorkspaceService.cs` にハードコーディングされている対象のメッセージを、すべて `Properties.Resources.XXX` の呼び出しに置換する。

## 4. エラーメッセージ定義要件
実装コード内にはメッセージ文字列を直接記述せず、`Properties/Resources.resx` に以下のエラーメッセージ（またはそれに準ずる定数）を定義し、条件に応じて正確に表示すること。
